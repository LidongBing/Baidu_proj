...
public class ExtractClassifyExample {
	...
	public static void printHelp() {
		...
		System.out.println("Please give six args, labelMapFile, solutionFile, outputFile,");
		System.out.println("      topN, thre, and singleLabel ");
		System.out.println("labelMapFile: contains the label ID map, in format 'symptom\t id'");
		System.out.println("solutionFile: contains the solution from SSL step");
		System.out.println("outputFile: the output file");
		System.out.println("topN: using the top N solutions in solutionFile, topN>=-1. if topN=-1, use all. If > 0, thre=0");
		System.out.println("thre: using the solutions with score >= \'thre\' in solutionFile, >= 0. If > 0, topN=0");
		System.out.println("singleLabel: Y if it is single label classifiation, otherwise N");
	...
	public void generateTrainingFile(String outfileName, String single) throws IOException {
		BufferedWriter bw = new BufferedWriter(new FileWriter(outfileName));
		ArrayList<String> sols = null;
		for (String label : labels) {

			if (!single.equals("N") && !single.equals("Y")) {
				System.out.println("ERROR: Single label 'S' or multiple label 'Y'?");
				System.exit(0);
			}

			if (single.equals("N")) {
				if (topN == 0 && thre != 0)
					sols = parsedSol.getSolsByThre(label, thre);
				else if (topN != 0 && thre == 0) {
					if (topN == -1)
						topN = Integer.MAX_VALUE;
					sols = parsedSol.getSolsByTop(label, topN);
				}
				else {
				...
				if (topN == 0 && thre != 0)
					sols = parsedSol.getSolsAppearingOnceByThre(label, thre);
				else if (topN != 0 && thre == 0) {
					if (topN == -1)
						topN = Integer.MAX_VALUE;
					sols = parsedSol.getSolsAppearingOnceByTop(label, topN);
				}
				else {
					System.out.println("ERROR: By TOP of by THRE?");
					System.exit(0);
			...
			for (String sol : sols) {
				...
				bw.write(sol + "\t" + this.labelMap.get(label));
				bw.newLine();
			}
		}
		bw.close();
